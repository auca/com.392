...
int main(int argc, char **argv) {
    ...
    while (true) {
        if (rank != 0) {
            ...
            MPI_Recv(&token, 1, MPI_INT, previous_neighbour, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);

            printf("Process %d out of %d got the token %d from process %d\n", rank, world_size, token, previous_neighbour);
        }

        int next_neighbour = (rank + 1) % world_size;
        printf("Process %d out of %d is sending the token %d to process %d\n", rank, world_size, token, next_neighbour);
